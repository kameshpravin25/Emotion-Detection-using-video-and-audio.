    ...
    try:
        if not video_active:
            ...
            if not video_capture.isOpened():
                return jsonify({"status": "Error: Could not open camera"}), 500
            video_active = True
            return jsonify({"status": "Video started successfully"})
        return jsonify({"status": "Video already running"})
    except Exception as e:
        return jsonify({"status": f"Error starting video: {str(e)}"}), 500
...
def stop_video():
    ...
    try:
        if video_active:
            ...
            if video_capture:
                video_capture.release()
            return jsonify({"status": "Video stopped successfully"})
        return jsonify({"status": "Video not running"})
    except Exception as e:
        return jsonify({"status": f"Error stopping video: {str(e)}"}), 500
...
def start_audio():
    ...
    try:
        if detector is None:
            ...
            return jsonify({"status": "Audio recording started successfully"})
        return jsonify({"status": "Already recording audio"})
    except Exception as e:
        return jsonify({"status": f"Error starting audio: {str(e)}"}), 500
...
def stop_audio():
    ...
    try:
        if detector is not None:
            detector.stop()
            detector = None
            return jsonify({"status": "Audio recording stopped successfully"})
        return jsonify({"status": "Not recording audio"})
    except Exception as e:
        return jsonify({"status": f"Error stopping audio: {str(e)}"}), 500
